...
cafe.order = async function(io, socket, itemList, itemCount, instructions, payingInPerson, customerId) { //Create New Cafe Order
    const platform = await setup(Platform);
    if (!platform.purchasable) {return console.log("Platform is not open to purchases");}
    const cafe = await setup(Market); //Collect data on cafe to figure out whether it's open or not
    if (!cafe) {return console.log('error accessing cafe');}
