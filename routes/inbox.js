...
router.get('/:id', middleware.isLoggedIn, (req, res) => {
    (async () => {
        ...
        await message.populate({path: 'sender', select: 'username'}).populate({
            path: 'recipients',
            select: 'username'
        }).populate({path: 'read', select: 'username'}).populate({path: 'replies.sender'}).execPopulate();
...
router.put('/mark-all', middleware.isLoggedIn, (req, res) => {
    ...
    }, {$push: {read: req.user._id}}, (err, result) => {
        if (err) {
