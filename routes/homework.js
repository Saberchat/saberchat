...
router.delete("/:id", middleware.isLoggedIn, middleware.isFaculty, middleware.memberOfCourse, (req, res) => {
  (async() => {
    const course = await Course.findOne({_id: req.params.id, joinCode: req.body.joinCode}).populate("tutors.tutor").populate("tutors.students").populate("tutors.rooms.student");
    if (!course) {
      req.flash("error", "Unable to find course");
      return res.redirect("back");
    }
