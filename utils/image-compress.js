...
const compressImage = async function(buffer) { //Compess image file
    return imagemin.buffer(buffer, { //Return buffer for compressed image
        plugins: [
            imageminJPEGTran(),
...
module.exports.autoCompress = async function(fileName, fileBuffer) { //Evaluate file and compress either way
    ...
    if([".png", ".jpg", ".jpeg"].includes(ext)) { //Check for image filetype
        try { //Attempt compression
            if (process.env.DEBUG === "true") {console.log("DEBUG: attempting to compress image...");}
            ...
            if (process.env.DEBUG === "true") {console.log("Image compression successful")}
        ...
        try {
            ...
            let compressedSize = compressedBuffer.byteLength;
            if (process.env.DEBUG === "true") {console.log("Video compression successful")}
            return compressedBuffer;
        } catch(e) {
