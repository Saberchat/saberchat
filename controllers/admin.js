...
controller.updateBalances = async function(req, res) {
    ...
    if (!platform.purchasable) { await sendGridEmail(user.email, "Balance Update", `<p>Hello ${user.firstName},</p><p>Your balance has been updated! Visit ${platform.url} to view all new items you have access to.}</p>`, false); }
