...
controller.orderForm = async function(req, res) {
    ...
    for (let category of categories) {
        if (category.items.length > 0) {
            ...
            quicksort(sortedCategory.items, 0, sortedCategory.items.length-1, "name"); //Call quicksort with item 'name' property
            ...
            sortedCategories.push(sortedCategory); //Add all sorted items to category list
        }
    }
    ...
    const frequentItems = sortByPopularity(Array.from(orderedItems.values()), "totalOrdered", "created_at", ["item"]).popular;
    ...
    if (req.query.order) { //Display ordering form
        if (req.user.tags.includes("Cashier")) { //At the moment, only cashiers may place orders
            if (!platform.purchasable || !req.user) {
                req.flash('error', `This feature is not enabled on ${platform.name} Saberchat`); //If ordering is not enabled
                return res.redirect('back');        
            }
...
controller.sortItems = async function(req, res) {
    ...
    for (let setting of [ //Iterate through each setting combination and sort accordingly
        ...
        if (req.body.setting == setting[0]) { //Iterate through each setting 
            quicksort(items, 0, items.length-1, setting[1]); //Sort based on provided setting
            if (setting[2]) {items.reverse();} //Reverse if display requires most->least
...
controller.searchCustomers = async function(req, res) {
    ...
    for (let user of users) { //Iterate through usernames and search for matches
        if (`${user.firstName} ${user.lastName} ${user.username}`.toLowerCase().includes(req.body.text.toLowerCase())) {
            customers.push({ //Add user to array, using username as display, and id as id value
...
controller.order = async function(req, res) {
    ...
    for (let item of orderedItems) { //Update items' availability and order count
        if (platform.displayAvailability && item.displayAvailability) { //If availability is supposed to be displayed
            ...
            item.orderCount += parseInt(req.body[item.name]);
            await item.save();
        }
    }
...
controller.processAll = async function(req, res) {
    ...
    for (let order of orders) { //Iterate through orders, update their data, and send messages to their customers
        ...
        if (!notif) {
            req.flash("error", "Unable to create notification");
            return res.redirect("back");
        }
    ...
    req.flash("success", "Successfully processed all orders!")
    return res.redirect("/shop/manage?orders=true");
}
...
controller.deleteOrder = async function(req, res) {
    ...
    //For each of the order"s items, add the number ordered back to that item.
    //(If there are 12 available quesadillas and the  user ordered 3, there are now 15)
...
controller.newItem = async function(req, res) {
    ...
    return res.render("shop/newOrderItem", {platform, shop, categories, data: platform.features[objectArrIndex(platform.features, "route", "shop")]});
}
...
controller.createItem = async function(req, res) {
    ...
    //Default is 0, but if admin opts not to display, set to 10 so it can still be ordered
...
controller.viewItem = async function(req, res) {
    ...
    return res.render("shop/show", {platform, shop, categories, item, fileExtensions, data: platform.features[objectArrIndex(platform.features, "route", "shop")]});
}

//Update or upvote item
controller.updateItem = async function(req, res) {
    if (req.body.name) {return controller.updateItemInfo(req, res)}; //If there is an attached form with a 'name' attribute, update item info.
    return controller.upvoteItem(req, res); //Otherwise, upvote (same route prefix)
}
...
controller.deleteItem = async function(req, res) {
    ...
    for (let order of orders) { //If the order includes this item, remove the item from that order"s item list
        for (let i of order.items) {
            if (!i.item) {order.items.splice(i, 1);}
        }
...
controller.manage = async function(req, res) {
    ...
    if (req.query.data) { //If route calls to display shop statistics
        ...
        data.market = market;
         data.data = platform.features[objectArrIndex(platform.features, "route", "shop")]; //Access full shop display data
        return res.render("shop/data", data);
    }

    return controller.manageShop(req, res); //If route calls to display regular management
}
...
controller.updateSettings = async function(req, res) {
    ...
    const items = await Item.find({});
    if (!items) {
        req.flash("error", "Could not access items");
        return res.redirect("back");
    }
    ...
    for (let item of items) {
        for (let tag of item.tags) {
            if (!shop.itemTags.includes(tag)) {item.tags.splice(item.tags.indexOf(tag), 1);}
        }
        await item.save();
    }
...
controller.createCategory = async function(req, res) {
    ...
    for (let t of categories) { //Now that we"ve created the category, we have to remove the newly selected items from all other categories
        for (let i = 0; i < t.items.length; i++) {
            if (req.body[t.items[i].toString()]) {t.items.splice(i, 1);}
        }
        await t.save();
    }
    ...
    for (let item of items) { //If the item is selected, add it to this category (now that we"ve removed it from all other categories)
        if (req.body[item._id.toString()]) {category.items.push(item);}
    }
...
controller.updateCategory = async function(req, res) {
    ...
    for (let item of category.items) {
        if (!req.body[item._id.toString()]) {other.items.push(item);} //Item is no longer checked; move it to "Other"
    }
    await other.save();
...
controller.deleteCategory = async function(req, res) {
    ...
    removeIfIncluded(shop.categories, category._id); //Remove from the official shop's category list
    await shop.save();
    for (let item of category.items) {other.items.push(item);} //Move all items to "other" category
    await other.save();
...
controller.upvoteItem = async function(req, res) {
    const item = await Item.findById(req.params.id);
    if (!item) {return res.json({error: "Error upvoting item"});}
    ...
    item.upvotes.push(req.user._id); //If not, add user's id to upvote list
    await item.save();
...
controller.updateItemInfo = async function(req, res) {
    if (!(req.user.tags.includes("Cashier"))) { //Check for Cashier Permission
        req.flash("error", "You do not have permission to do that");
        return res.redirect("back");
    }
...
controller.manageShop = async function(req, res) {
    ...
    for (let category of categories) { //Sort displayed categories alphabetically
        sortedCategory = category;
        quicksort(sortedCategory.items, 0, sortedCategory.items.length-1, "name");
        sortedCategories.push(sortedCategory);
    }
    return res.render("shop/manage", {platform, shop, categories: sortedCategories, data: platform.features[objectArrIndex(platform.features, "route", "shop")]});
}
...
controller.manageOrders = async function(req, res) {
    ...
    let viewPast = false;
    if(req.query.past) {viewPast = true;}

    // set defaults
    const now = new Date();
    let startDate = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    let endDate = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    endDate.setDate(endDate.getDate() + 1); // End datetime is technically beginning of next day
    ...
    return res.render("shop/orderDisplay", {
        ...
        data: platform.features[objectArrIndex(platform.features, "route", "shop")],
        viewPast: viewPast
    });
}
