const addEmail = function(event, version) { //Add an email to access list/blocked list
    ...
    sendPostReq(url, data, data => {
        ...
        if (data.success) { //If successful response, create new HTML element for email
            ...
            for (let email of data.emails) { //Iterate through new emails and add each one
                newEmail = document.createElement("li");
                newEmail.className = "list-group-item email shop";
                newEmail.id = `${email._id}`;
                newEmail.innerHTML = `${email.address} <div class="delete-email"> <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#modal-${email._id}" id="delete-button"><i class="fas fa-trash-alt"></i> Remove</button> <div class="modal fade" id="modal-${email._id}" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true"> <div class="modal-dialog mode"> <div class="modal-content mode"> <div class="modal-header mode"> <h5 class="modal-title" id="exampleModalLabel">Remove Email From Access List?</h5> <button type="button" class="close mode" data-dismiss="modal" aria-label="Close"> <span aria-hidden="true">&times;</span> </button> </div> <div class="modal-body mode"> If an account is currently using this email, it cannot be deleted. </div> <div class="modal-footer mode"> <button type="button" class="btn btn-secondary" data-dismiss="modal">No, Go Back</button> <button type="button" onclick="removeEmail(this)" id="remove-${email._id}" class="btn btn-danger">Yes, Remove Email</button></div></div></div></div></div>`;
