const like = function(button, object, route) { // Like an object
    ...
    sendPostReq(url, data, data => {
        if (data.success) {
            // If response involved removing a like, change button color based on page
            ...
            document.getElementsByClassName(`likeCount-${id}`)[0].innerText = data.likeCount;
        }
    });
}
...
const likeComment = function(button, object, route) { // Like a comment on an object
    ...
    sendPostReq(url, data, data => {
        if (data.success) {
            // If response involved removing a like, change button color
            if (data.success.includes("Removed a like")) {
                button.style.color = "grey";
