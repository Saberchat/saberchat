let starCounts = new Map(); //Tracks ratings for each tutor so they can be submitted at any time

const rate = ((button, tutorId) => {
    if (starCounts.has(tutorId)) {
        if (starCounts.get(tutorId) == parseInt(button.id)) { //If clicked star is already the number of stars, subtract 1 (most out-of-5 review systems I have found work this way)
...
const submitRating = ((button, location) => {
    ...
    if (!starCounts.has(tutorId)) { //If person has not added any stars, make the review 0 stars
